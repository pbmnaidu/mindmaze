 ...
 */
export async function fetchWorkDetail(workId: string, scope: ScopeParams): Promise<ApiResult<WorkDetailResponse>> {
  const routes = [
    `/work-detail${buildQuery({ work_id: workId, ...scope })}`,
    `/work-detail/${encodeURIComponent(workId)}${buildQuery({ ...scope })}`,
  ];
